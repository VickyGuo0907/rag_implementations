...
RETRIEVAL_DECISION_PROMPT = """Decide whether answering the following question requires \
retrieving external documents, or whether it can be answered accurately from general \
knowledge alone. Respond with ONLY a single number between 0.0 and 1.0 -- 0.0 means \
retrieval is definitely not needed, 1.0 means retrieval is essential.

Question: {question}
Retrieval necessity score:"""

RELEVANCE_GRADE_PROMPT = """For each numbered passage below, judge how relevant it is \
to answering the question. Respond with ONLY the scores, one per line, in the same order, \
each between 0.0 (not relevant) and 1.0 (highly relevant). No other text.
...
SUPPORT_GRADE_PROMPT = """Judge how well the following answer is supported by the \
provided context. An answer is well-supported if every claim in it can be verified \
against the context. Respond with ONLY a single number between 0.0 (not supported / \
hallucinated) and 1.0 (fully supported).
...
REFINE_QUERY_PROMPT = """The following answer to a question was not well-supported by \
the retrieved context, suggesting the search didn't find the right documents. Generate a \
...
NO_RETRIEVAL_PROMPT = """Answer the following question directly using your own knowledge. \
...
class SelfRAGLlamaIndex(BaseRAG):
    ...
    def _grade_relevance(self, question: str, candidates: list) -> tuple[list, list[float]]:
        ...
        response = self.llm.complete(RELEVANCE_GRADE_PROMPT.format(question=question, passages=passages_text))  # noqa: E501
        scores = _parse_scores(response.text, count=len(candidates))
        relevant = [node for node, score in zip(candidates, scores, strict=True) if score >= self.relevance_threshold]  # noqa: E501
    ...
    def _grade_support(self, question: str, nodes: list, answer: str) -> float:
        context = "\n\n---\n\n".join(node.get_content() for node in nodes)
        response = self.llm.complete(SUPPORT_GRADE_PROMPT.format(context=context, question=question, answer=answer))  # noqa: E501
        return _parse_score(response.text, default=0.5)

    def _refine_query(self, question: str, weak_answer: str) -> str:
        response = self.llm.complete(REFINE_QUERY_PROMPT.format(question=question, answer=weak_answer))  # noqa: E501
        return _strip_thinking(response.text)
    ...
    def _query(self, question: str) -> RAGResult:
        ...
        logger.debug(f"[SelfRAG/LI] retrieval_necessity={retrieval_score:.2f} → needs_retrieval={needs_retrieval}")  # noqa: E501
        ...
        for i in range(self.max_iterations):
            ...
            logger.debug(
                f"[SelfRAG/LI] iteration {i + 1}: {len(relevant_nodes)}/{len(candidates)} relevant, "  # noqa: E501
